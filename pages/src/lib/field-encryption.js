...
const saltGeneratorWrapper = (iv) => {
  if (iv instanceof Buffer) {
    if (iv.length !== 16) {
      throw new Error(
        "Invalid salt provided, please ensure that the salt is a Buffer of length 16"
      );
    }
    return iv;
  }
  ...
  if (typeof iv === "string" || iv instanceof String) {
    if (iv.length !== 16) {
      throw new Error(
        "Invalid salt, please ensure that the salt is a string of length 16"
      );
    }
  ...
  throw new Error(
    "Invalid salt, please ensure that the salt is either a string or a Buffer of length 16"
  );
};
    ...
    for (const field of fields) {
      ...
      if (!obj[encryptedFieldName] && typeof fieldValue !== "undefined") {
        if (typeof fieldValue === "string") {
          ...
          const value = encryptionStrategy(
  ...
  function updateHook(_next) {
    ...
    for (const field of fieldsToEncrypt) {
      ...
      const encryptedFieldValue =
        this._update.$set[encryptedFieldName] ||
        this._update[encryptedFieldName];

      if (!encryptedFieldValue && plainTextValue) {
        const updateObj = {};
        if (
          typeof plainTextValue === "string" ||
          plainTextValue instanceof String
        ) {
          const encryptedData = encryptionStrategy(
            plainTextValue,
            secret(),
            saltGenerator
          );
          ...
          updateObj[encryptedFieldData] = encryptionStrategy(
        ...
        this.update(
          {},
          Object.keys(this._update.$set).length > 0
            ? { $set: updateObj }
            : updateObj
        );
      }
